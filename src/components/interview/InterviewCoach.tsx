  ...
  if (!jobDescription || !cvText) {
    return (
      <div className="text-center py-8">
        <Sparkles className="mx-auto h-8 w-8 text-[#45454e] mb-3" />
        <p className="text-sm text-[#63636e]">Add a job description to generate interview prep.</p>
      </div>
    )
  }
  ...
  if (!result) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-[#63636e] mb-4">Generate technical questions, behavioral questions, and company preparation.</p>
      ...
      <Tabs defaultValue="technical">
              ...
              <button onClick={() => setExpandedQuestion(expandedQuestion === i ? null : i)} className="flex items-start justify-between w-full text-left">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-sm font-medium text-[#fafafa]">{q.question}</span>
                  </div>
                ...
                {expandedQuestion === i ? <ChevronUp className="h-4 w-4 text-[#45454e]" /> : <ChevronDown className="h-4 w-4 text-[#45454e]" />}
              </button>
              {expandedQuestion === i && q.sample_answer && (
                <div className="mt-3 ml-7 p-3 rounded-lg bg-[#0f0f12] border border-[#1e1e24]">
                  <p className="text-xs text-[#63636e] font-medium mb-1">Sample Answer:</p>
                  <p className="text-sm text-[#a0a0ab]">{q.sample_answer}</p>
                </div>
              )}
            </div>
          ))}
            ...
            <div key={i} className="p-4 rounded-xl border border-[#1e1e24] bg-[#16161a]">
              <button onClick={() => setExpandedBehavioral(expandedBehavioral === i ? null : i)} className="flex items-start justify-between w-full text-left">
                <div className="flex-1">
                  ...
                  <span className="text-xs text-[#45454e] ml-7">STAR: {q.type}</span>
                </div>
                {expandedBehavioral === i ? <ChevronUp className="h-4 w-4 text-[#45454e]" /> : <ChevronDown className="h-4 w-4 text-[#45454e]" />}
              </button>
              {expandedBehavioral === i && (
                <div className="mt-3 ml-7 space-y-2">
                  {["situation", "task", "action", "result"].map((field) => (
                    <div key={field} className="p-2 rounded bg-[#0f0f12] border border-[#1e1e24]">
                      <span className="text-xs font-medium text-[#63636e] capitalize">{field}:</span>
                      <p className="text-sm text-[#a0a0ab]">{q[field]}</p>
                    </div>
                  ))}
                </div>
              )}
        ...
        <TabsContent value="company">
          {result.company_preparation && (
            <div className="p-4 rounded-xl border border-[#1e1e24] bg-[#16161a] space-y-4">
              <div>
                <h4 className="text-xs font-medium text-[#63636e] uppercase tracking-wider mb-2">Interview Format</h4>
                <p className="text-sm text-[#a0a0ab]">{result.company_preparation.common_interview_format}</p>
              </div>
              <div>
                <h4 className="text-xs font-medium text-[#63636e] uppercase tracking-wider mb-2">Key Areas to Review</h4>
                <ul className="space-y-1">
                  {result.company_preparation.key_areas_to_review?.map((area: string, i: number) => (
                    <li key={i} className="flex items-center gap-2 text-sm text-[#a0a0ab]">
              ...
              <div>
                <h4 className="text-xs font-medium text-[#63636e] uppercase tracking-wider mb-2">Questions to Ask</h4>
                <ul className="space-y-1">
